...
class ClaudeAPI {
    ...
    /// Initialize with a direct API key (calls api.anthropic.com directly)
    /// or a proxy URL (calls Cloudflare Worker, no key needed).
    init(apiKey: String? = nil, proxyURL: String? = nil, model: String = "claude-sonnet-4-6") {
        if let apiKey, !apiKey.isEmpty {
            ...
            fatalError("ClaudeAPI requires either apiKey or proxyURL")
        }
        self.model = model
