...
final class MenuBarPanelManager: NSObject {
    ...
    /// Returns the daytora-chan color icon for the menu bar.
    /// Falls back to the original triangle drawing if the asset is missing.
    private func makeClickyMenuBarIcon() -> NSImage {
        let iconSize: CGFloat = 18

        if let asset = NSImage(named: "daytora-chan") {
            asset.size = NSSize(width: iconSize, height: iconSize)
            asset.isTemplate = false
            return asset
        }

        // Fallback: draw the clicky triangle
