...
画面を指差しながら、キャラクターボイスで教えてくれるAIメンター。

macOSメニューバーに常駐して、Control+Optionキーを長押しするだけで質問できます。「ここをクリックして」と画面上の要素を直接指し示しながら声で解説。プログラミング、デザイン、ライティング、語学——ジャンルを問わず対応します。
...
```bash
# VOICEVOXエンジンをセットアップ（開発者のみ。要: VOICEVOX.appインストール済み）
# ※ DMGでインストールするユーザーはこの手順は不要（エンジンはアプリにバンドル済み）
